...
export const insertMultipleProducts = async (Product: Products[]): Promise<void> => {
...
export const insertOneSales = async (sales: Sales): Promise<number | undefined> => {
    try {
        const res = await executeQuery(
            'INSERT INTO sales (product_id, quantity_sold, sale_date, total_price) VALUES ($1, $2, $3, $4) RETURNING sale_id',
            [sales.product_id, sales.quantity_sold, sales.sale_date, sales.total_price]
        );
...
export const JoinSalesWithProductDetails = async (): Promise<SalesWithProduct[]> => {
    try {
        const res = await executeQuery(`
